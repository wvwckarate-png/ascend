...
setLoading(false);
  ...
  const navigateToResource = (task: Task) => {
    ...
    if (task.resource_type === 'flashcard_deck') router.push(`/matthew/flashcards?deckId=${task.resource_id}`);
    if (task.resource_type === 'study_guide')    router.push(`/matthew/study?guideId=${task.resource_id}`);
    if (task.resource_type === 'practice_exam')  router.push(`/matthew/practice-exam?examId=${task.resource_id}`);
  };
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
          ...
          <Link href="/matthew/profile" style={{ textDecoration: 'none' }}>
            <div style={{ width: 44, height: 44, borderRadius: '50%', background: light, overflow: 'hidden', border: `2px solid ${color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <img src="/eagle.png" alt="Eagle" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
            </div>
          </Link>
        </div>
...
<div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
  <svg width="20" height="20" viewBox="0 0 28 28" fill="none">
    ...
    <rect x="8" y="15" width="4" height="4" rx="1" fill="white" opacity="0.7"/>
        ...
        {(() => {
          ...
          return (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2.5, textTransform: 'uppercase', color: '#C4C1D4', marginBottom: 10 }}>Upcoming Exams</div>
              <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4, scrollbarWidth: 'none' }}>
                {upcoming.map(e => {
                  ...
                  return (
                    <div
                      key={e.id}
                      onClick={() => router.push(`/matthew/classes`)}
        ...
        <div style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 18, padding: '18px 20px', marginBottom: 16, boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <Link href="/matthew/calendar" style={{ fontSize: 10, fontWeight: 700, color, textDecoration: 'none' }}>+ Add</Link>
            </div>
          </div>
        ...
        <div style={{ background: 'linear-gradient(135deg, #7B6FA0, #5A5078)', borderRadius: 16, padding: '14px 18px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
          ...
          <div onClick={() => router.push('/matthew/flashcards')} style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 14, padding: '16px 8px', textAlign: 'center', cursor: 'pointer', boxShadow: '0 1px 4px rgba(29,27,38,0.06)', transition: 'transform 0.15s' }} onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-2px)'} onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.transform = 'translateY(0)'}>
          ...
          <div onClick={() => router.push('/matthew/practice-exam')} style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 14, padding: '16px 8px', textAlign: 'center', cursor: 'pointer', boxShadow: '0 1px 4px rgba(29,27,38,0.06)', transition: 'transform 0.15s' }} onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-2px)'} onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.transform = 'translateY(0)'}>
      ...
      {showUpload && <UploadResourceModal student="matthew" onClose={() => setShowUpload(false)} />}
            ...
            <div style={{ background: '#FFFFFF', borderRadius: 22, padding: '24px 20px 20px', width: '100%', maxWidth: 480, boxShadow: '0 8px 40px rgba(29,27,38,0.18)', maxHeight: '80vh', overflowY: 'auto' }}>
              ...
              {dayExams.map(exam => (
                <div key={exam.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 12, background: '#FDF2F2', border: '1.5px solid rgba(196,120,120,0.2)', marginBottom: 8 }}>
                  ...
                  <div style={{ fontSize: 10, fontWeight: 700, color: '#C47878', background: '#FDF2F2', padding: '2px 8px', borderRadius: 999, border: '1px solid rgba(196,120,120,0.3)' }}>Exam</div>
                </div>
              ))}
              ...
              <div style={{ marginTop: 16, paddingTop: 14, borderTop: '1px solid #F3F1EC', textAlign: 'center' }}>
                <Link href="/matthew/calendar" onClick={() => setSelectedDay(null)} style={{ fontSize: 12, fontWeight: 700, color, textDecoration: 'none' }}>Open in Calendar →</Link>
              </div>
            </div>
          </div>
        );
