...
export default function MatthewClasses() {
  ...
  useEffect(() => {
    const load = async () => {
      const { data: active }   = await supabase.from('classes').select('id, name, semester, professor').eq('student_id', 'matthew').eq('is_active', true).order('created_at', { ascending: false });
      const { data: inactive } = await supabase.from('classes').select('id, name, semester, professor').eq('student_id', 'matthew').eq('is_active', false).order('created_at', { ascending: false });
