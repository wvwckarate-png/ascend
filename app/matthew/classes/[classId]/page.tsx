...
export default function MatthewClassBinder() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAFAF8' }}>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '28px 20px 80px' }}>
        ...
        {cls && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 28 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: '#9E9BB0' }}>
                {[cls.semester, cls.professor].filter(Boolean).join(' · ')}
              </div>
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          ...
          <button
            ...
            style={{ padding: '6px 14px', borderRadius: 999, background: '#EDE9F7', border: 'none', color: '#7B6FA0', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}
          >
            + Add Folder
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ background: '#FFFFFF', border: '1.5px dashed #C4C1D4', borderRadius: 18, padding: '40px 20px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 13, color: '#9E9BB0', marginBottom: 20 }}>Add a folder for each exam or unit in this class.</div>
            <button
              onClick={() => setShowAdd(true)}
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    <div style={{ width: 40, height: 40, borderRadius: 11, background: '#EDE9F7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color: '#7B6FA0', flexShrink: 0 }}>
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        {folder.exam_date && (
                          <span style={{ fontSize: 11, color: '#9E9BB0' }}>{formatDate(folder.exam_date)}</span>
                        )}
                        {countdown && (
      ...
      {showAdd && (
        <div
          onClick={e => { if (e.target === e.currentTarget) { setShowAdd(false); setNewName(''); setNewDate(''); }}}
        ...
        >
          <div style={{ background: '#FFFFFF', borderRadius: '22px 22px 0 0', padding: '24px 20px 36px', width: '100%', maxWidth: 580, boxShadow: '0 -8px 40px rgba(29,27,38,0.12)' }}>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: '#9E9BB0', marginBottom: 6, display: 'block' }}>Folder Name</label>
              <input
                autoFocus
                value={newName}
                onChange={e => setNewName(e.target.value)}
                onKeyDown={e => { if (e.key === 'Enter' && newName.trim()) handleAddFolder(); }}
                placeholder="e.g. Exam 2, Midterm, Final..."
            ...
            <div style={{ marginBottom: 22 }}>
              <label style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: '#9E9BB0', marginBottom: 6, display: 'block' }}>Exam Date (optional)</label>
              <input
                type="date"
                value={newDate}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button
                ...
                style={{ flex: 2, padding: '12px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg, #7B6FA0, #5A5078)', color: 'white', fontSize: 13, fontWeight: 800, cursor: 'pointer', fontFamily: 'var(--font-jakarta)', opacity: !newName.trim() || saving ? 0.4 : 1 }}
              >
