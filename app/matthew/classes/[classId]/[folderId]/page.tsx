...
export default function MatthewBinder() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Weak spots
      const weak: WeakSpot[] = [];
      const { data: deckRows } = await supabase.from('flashcard_decks').select('id').eq('folder_id', folderId).eq('student_id', 'matthew');
      if (deckRows && deckRows.length > 0) {
        const deckIdList = deckRows.map(d => d.id);
        const { data: items } = await supabase.from('flashcard_items').select('card_id, times_seen, times_missed, is_retired').in('deck_id', deckIdList).eq('student_id', 'matthew').gte('times_seen', 3).eq('is_retired', false);
        if (items) {
      ...
      const { data: examWeak } = await supabase.from('practice_exam_items').select('question_text, question_type').eq('folder_id', folderId).eq('student_id', 'matthew').eq('is_correct', false).in('question_type', ['mc', 'tf']);
  ...
  const handleSaveDate = async () => {
    ...
    // Update linked calendar task via folder_id
    await supabase.from('tasks').update({ due_date: dateToSave }).eq('folder_id', folderId).eq('student_id', 'matthew');
                  ...
                  {editingDate ? (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
                      <input
                        ...
                        style={{ padding: '5px 10px', border: '1.5px solid #7B6FA0', borderRadius: 8, fontFamily: 'var(--font-jakarta)', fontSize: 12, color: '#1D1B26', background: '#FAFAF8', outline: 'none' }}
                      />
                      <div style={{ display: 'flex', gap: 6 }}>
                        ...
                        <button onClick={handleSaveDate} disabled={savingDate} style={{ fontSize: 11, fontWeight: 700, color: 'white', background: '#7B6FA0', border: 'none', borderRadius: 7, padding: '4px 10px', cursor: 'pointer', fontFamily: 'var(--font-jakarta)', opacity: savingDate ? 0.6 : 1 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        {folder.exam_date
                          ...
                          : <div style={{ fontSize: 12, color: '#C4C1D4' }}>No date set</div>
                        }
                      ...
                      {dateSaved && (
                        <div style={{ fontSize: 11, fontWeight: 700, color: '#5FAD8E' }}>✓ Date updated</div>
                      )}
                    </div>
                  )}
                </div>
              ...
              {tab === 'cards' && (
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
                    ...
                    <button onClick={() => router.push(`/matthew/flashcards?folderId=${folderId}&folderName=${encodeURIComponent(folder.name)}`)} style={{ padding: '9px 18px', borderRadius: 999, background: '#7B6FA0', border: 'none', color: 'white', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
                      {folderDeck ? 'New Deck' : 'Generate'}
                    </button>
                  </div>
                  {folderDeck ? (
                    <div onClick={() => router.push(`/matthew/flashcards`)} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '18px 20px', background: '#FAFAF8', borderRadius: 14, border: '1.5px solid #E8E5F0', cursor: 'pointer' }}>
              ...
              {tab === 'exam' && (
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
                    ...
                    <button onClick={() => router.push(`/matthew/practice-exam?folderId=${folderId}&folderName=${encodeURIComponent(folder.name)}`)} style={{ padding: '9px 18px', borderRadius: 999, background: '#7B6FA0', border: 'none', color: 'white', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)', flexShrink: 0 }}>
                      {folderExam ? 'New Exam' : 'Generate'}
                    </button>
                  </div>
                  {folderExam ? (
                    <div onClick={() => router.push('/matthew/practice-exam')} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '18px 20px', background: '#FAFAF8', borderRadius: 14, border: '1.5px solid #E8E5F0', cursor: 'pointer' }}>
                      <div style={{ width: 48, height: 48, borderRadius: 13, background: '#EDE9F7', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    ...
                    <div style={{ textAlign: 'center', padding: '40px 20px', border: '2px dashed #E8E5F0', borderRadius: 14 }}>
                      ...
                      <button onClick={() => hasResources ? router.push(`/matthew/practice-exam?folderId=${folderId}&folderName=${encodeURIComponent(folder.name)}`) : setTab('resources')} style={{ padding: '11px 24px', borderRadius: 999, background: hasResources ? '#7B6FA0' : '#F3F1EC', border: 'none', color: hasResources ? 'white' : '#9E9BB0', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
              ...
              {tab === 'guide' && (
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
                    ...
                    <button onClick={() => router.push(`/matthew/study?folderId=${folderId}&folderName=${encodeURIComponent(folder.name)}`)} style={{ padding: '9px 18px', borderRadius: 999, background: '#7B6FA0', border: 'none', color: 'white', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
                      {folderGuide ? 'New Guide' : 'Generate'}
                    </button>
                  </div>
                  {folderGuide ? (
                    <div onClick={() => router.push(`/matthew/study?guideId=${folderGuide.id}&folderId=${folderId}&folderName=${encodeURIComponent(folder.name)}`)} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '18px 20px', background: '#FAFAF8', borderRadius: 14, border: '1.5px solid #E8E5F0', cursor: 'pointer' }}>
                      <div style={{ width: 48, height: 48, borderRadius: 13, background: '#EDE9F7', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    ...
                    <div style={{ textAlign: 'center', padding: '40px 20px', border: '2px dashed #E8E5F0', borderRadius: 14 }}>
                      ...
                      <button onClick={() => hasResources ? router.push(`/matthew/study?folderId=${folderId}&folderName=${encodeURIComponent(folder.name)}`) : setTab('resources')} style={{ padding: '11px 24px', borderRadius: 999, background: hasResources ? '#7B6FA0' : '#F3F1EC', border: 'none', color: hasResources ? 'white' : '#9E9BB0', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
        ...
        <div onClick={e => { if (e.target === e.currentTarget) { setShowUpload(false); resetUpload(); }}} style={{ position: 'fixed', inset: 0, background: 'rgba(29,27,38,0.5)', backdropFilter: 'blur(4px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: '#FFFFFF', borderRadius: 22, padding: '24px 20px 32px', width: '100%', maxWidth: 480, boxShadow: '0 8px 40px rgba(29,27,38,0.18)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8, marginBottom: 20 }}>
              {FILE_TYPES.map(ft => (
                <div key={ft.key} onClick={() => { setUpType(ft.key); setUpFile(null); if (fileInputRef.current) fileInputRef.current.value = ''; }} style={{ padding: '12px 6px', borderRadius: 12, border: `1.5px solid ${upType === ft.key ? '#7B6FA0' : '#E8E5F0'}`, background: upType === ft.key ? '#EDE9F7' : '#FAFAF8', cursor: 'pointer', textAlign: 'center', transition: 'all 0.15s' }}>
