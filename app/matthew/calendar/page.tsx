...
export default function MatthewCalendar() {
  ...
  const handleAddTask = async () => {
    ...
    const { error } = await supabase.from('tasks').insert({
      student_id: 'matthew', title: newTitle.trim(), due_date: newDate,
      due_time: newTime || null, task_type: newType, class_name: newClass || null, completed: false
    });
    if (!error) {
      const { data: fresh } = await supabase.from('tasks').select('*').eq('student_id', 'matthew').order('due_date', { ascending: true });
        ...
        <div onClick={e => { if (e.target === e.currentTarget) setShowAdd(false); }} style={{ position: 'fixed', inset: 0, background: 'rgba(29,27,38,0.5)', backdropFilter: 'blur(4px)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div style={{ background: '#FFFFFF', borderRadius: '22px 22px 0 0', padding: '24px 20px 44px', width: '100%', maxWidth: 580, boxShadow: '0 -8px 40px rgba(29,27,38,0.15)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={handleAddTask} disabled={!newTitle.trim() || !newDate || saving} style={{ flex: 2, padding: '13px', borderRadius: 12, border: 'none', background: !newTitle.trim() ? '#F3F1EC' : 'linear-gradient(135deg, #7B6FA0, #5A5078)', color: !newTitle.trim() ? '#C4C1D4' : 'white', fontSize: 13, fontWeight: 800, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>{saving ? 'Saving...' : 'Add Task'}</button>
            </div>
          </div>
        </div>
      )}
