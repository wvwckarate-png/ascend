      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '20px 20px 80px' }}>
        ...
        <div style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 18, padding: '16px', marginBottom: 16, boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', marginBottom: 8 }}>
            {DAYS.map(d => (
              <div key={d} style={{ textAlign: 'center', fontSize: 10, fontWeight: 700, color: '#C4C1D4', letterSpacing: 0.5, padding: '4px 0' }}>{d}</div>
            ))}
          </div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    background: isSelected ? '#7B6FA0' : isToday ? '#EDE9F7' : 'transparent',
                  }}
                >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <button onClick={() => setShowAddTask(true)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '7px 14px', borderRadius: 999, border: 'none', background: '#7B6FA0', color: 'white', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
            + Add Task
          </button>
        </div>
        ...
        {showAddTask && (
          <div style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 16, padding: '16px', marginBottom: 12, boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
            <input
              ...
              autoFocus
              style={{ width: '100%', padding: '10px 13px', border: '1.5px solid #E8E5F0', borderRadius: 10, fontFamily: 'var(--font-jakarta)', fontSize: 14, color: '#1D1B26', background: '#FAFAF8', outline: 'none', marginBottom: 8 }}
            />
            <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
              <input
                ...
                style={{ flex: 1, padding: '8px 10px', border: '1.5px solid #E8E5F0', borderRadius: 10, fontFamily: 'var(--font-jakarta)', fontSize: 13, color: '#1D1B26', background: '#FAFAF8', outline: 'none' }}
              />
              <input
                value={newTaskClass}
                onChange={e => setNewTaskClass(e.target.value)}
                placeholder="Class (optional)"
                style={{ flex: 2, padding: '8px 10px', border: '1.5px solid #E8E5F0', borderRadius: 10, fontFamily: 'var(--font-jakarta)', fontSize: 13, color: '#1D1B26', background: '#FAFAF8', outline: 'none' }}
              />
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={addTask} disabled={!newTaskTitle.trim() || saving} style={{ flex: 1, padding: '10px', borderRadius: 10, border: 'none', background: '#7B6FA0', color: 'white', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)', opacity: !newTaskTitle.trim() ? 0.4 : 1 }}>
                {saving ? 'Saving...' : 'Add Task'}
              </button>
              <button onClick={() => setShowAddTask(false)} style={{ padding: '10px 16px', borderRadius: 10, border: '1.5px solid #E8E5F0', background: '#FAFAF8', color: '#9E9BB0', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
                Cancel
              </button>
            </div>
          </div>
        ...
        {selectedTasks.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {selectedTasks.map(task => (
              <div key={task.id} style={{ background: '#FFFFFF', border: `1.5px solid ${task.completed ? '#E8E5F0' : '#E8E5F0'}`, borderRadius: 14, padding: '14px 16px', boxShadow: '0 1px 6px rgba(29,27,38,0.06)', display: 'flex', alignItems: 'center', gap: 12, opacity: task.completed ? 0.6 : 1 }}>
                {/* Checkbox */}
                <button
                  onClick={() => toggleComplete(task)}
                  style={{ width: 22, height: 22, borderRadius: 6, border: `2px solid ${task.completed ? '#5FAD8E' : '#C4C1D4'}`, background: task.completed ? '#5FAD8E' : 'transparent', cursor: 'pointer', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                />
