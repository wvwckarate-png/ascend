...
function MatthewPracticeExamInner() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const { data: examData } = await supabase.from('practice_exams').insert({ student_id: 'matthew', title, questions: parsed, responses: {}, status: 'in_progress', timer_seconds: timerEnabled ? timerMinutes * 60 : null }).select().single();
