  ...
  const generate = async () => {
    ...
    try {
      ...
      if (folderFiles.length > 0) {
        const prompt = `Generate ${count} practice exam questions from the uploaded study materials${topic.trim() ? ` about: ${topic}` : ''} (${modeLabel}). Return ONLY a JSON array with no markdown, no backticks, no explanation. Format: [{"front":"question","back":"detailed answer"}]. Pre-dental college level. Make questions challenging and specific.`;
        ...
        const prompt = `Generate ${count} practice exam questions for: ${topic} (${modeLabel}). Return ONLY a JSON array with no markdown, no backticks, no explanation. Format: [{"front":"question","back":"detailed answer"}]. Pre-dental college level. Make questions challenging and specific.`;
