    ...
    <div style={{ minHeight: '100vh', background: '#FAFAF8' }}>
      ...
      <main style={{ maxWidth: 600, margin: '0 auto', padding: '28px 20px 80px' }}>
        <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 13, color: '#9E9BB0' }}>Enter your class info. You can add your syllabus later.</div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={cardStyle}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {['AP', 'Honors', 'Dual Enrollment', 'Intro', 'Intermediate', 'Advanced'].map(l => (
                <button key={l} onClick={() => setClassLevel(classLevel === l ? '' : l)} style={{ padding: '8px 14px', borderRadius: 999, border: `1.5px solid ${classLevel === l ? '#7B6FA0' : '#E8E5F0'}`, background: classLevel === l ? '#7B6FA0' : '#FAFAF8', color: classLevel === l ? 'white' : '#9E9BB0', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
                  {l}
                </button>
              ))}
            </div>
          ...
          <div style={cardStyle}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {CLASS_FORMATS.map(f => (
                <button key={f} onClick={() => setClassFormat(f)} style={{ flex: 1, padding: '10px', borderRadius: 10, border: `1.5px solid ${classFormat === f ? '#7B6FA0' : '#E8E5F0'}`, background: classFormat === f ? '#7B6FA0' : '#FAFAF8', color: classFormat === f ? 'white' : '#9E9BB0', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
                  {f}
                </button>
              ))}
            </div>
          ...
          <div style={cardStyle}>
            ...
            <div onClick={() => document.getElementById('syllabus-matthew')?.click()} style={{ padding: '24px', borderRadius: 12, border: '2px dashed #E8E5F0', background: '#FAFAF8', textAlign: 'center', cursor: 'pointer' }}>
              <div style={{ fontSize: 24, marginBottom: 8 }}>📄</div>
              <div style={{ fontSize: 13, color: '#9E9BB0' }}>{file ? file.name : 'Tap to upload your syllabus PDF'}</div>
              <input id="syllabus-matthew" type="file" accept=".pdf" style={{ display: 'none' }} onChange={e => setFile(e.target.files?.[0] || null)} />
            </div>
          </div>
