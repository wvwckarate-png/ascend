    ...
    <div style={{ minHeight: '100vh', background: '#FAFAF8' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1D1B26, #3A3545)', padding: '28px 20px 24px' }}>
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          ...
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.45)' }}>Here's how your students are doing.</div>
        </div>
      </div>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '24px 20px 60px' }}>
        ...
        {/* Student cards */}
        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2.5, textTransform: 'uppercase', color: '#C4C1D4', marginBottom: 14 }}>Each Student</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 28 }}>
          {students.map((s) => (
            <Link key={s.id} href={`/${s.id}`} style={{ textDecoration: 'none' }}>
              <div style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 18, padding: '18px 20px', boxShadow: '0 1px 6px rgba(29,27,38,0.06)', display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#7B6FA0' }}>{s.goal}</div>
                </div>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#C4C1D4', flexShrink: 0 }}>View →</div>
              </div>
            </Link>
          ))}
        </div>
