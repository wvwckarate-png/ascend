    ...
    <div style={{ minHeight: '100vh', background: '#FAFAF8' }}>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '28px 20px 80px' }}>
        <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 13, color: '#9E9BB0' }}>Upload a PDF and Ascend will turn it into something amazing!</div>
        </div>
        ...
        {!studyGuide ? (
          <>
            <div style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 18, padding: '20px', marginBottom: 12, boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
              <div onClick={() => document.getElementById('pdf-upload-brynne')?.click()} style={{ padding: '40px 20px', borderRadius: 12, border: '2px dashed #E8E5F0', background: '#FAFAF8', textAlign: 'center', cursor: 'pointer' }}>
                ...
                <input id="pdf-upload-brynne" type="file" accept=".pdf" style={{ display: 'none' }} onChange={e => setFile(e.target.files?.[0] || null)} />
              </div>
            </div>
            {error && <p style={{ fontSize: 13, color: '#C47878', marginBottom: 12 }}>{error}</p>}
            <button onClick={handleGenerate} disabled={!file || loading} style={{ width: '100%', padding: '14px', borderRadius: 14, border: 'none', background: 'linear-gradient(135deg, #E8956D, #C4A882)', color: 'white', fontSize: 14, fontWeight: 800, cursor: 'pointer', fontFamily: 'var(--font-jakarta)', opacity: !file || loading ? 0.4 : 1 }}>
          ...
          <div style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 18, padding: '24px', boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              ...
              <button onClick={() => { setStudyGuide(''); setFile(null); setSaved(false); setShowNamePrompt(false); }} style={{ fontSize: 12, fontWeight: 700, color: '#E8956D', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>Make Another</button>
            </div>
            <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
              <button onClick={handlePrint} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 10, border: '1.5px solid #E8E5F0', background: '#FFF3E8', color: '#C4A882', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>🖨️ Print</button>
              <button onClick={handlePrint} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 10, border: '1.5px solid #E8E5F0', background: '#FFF3E8', color: '#C4A882', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>💾 Save as PDF</button>
            </div>
