...
function BrynneFlashcardsInner() {
  ...
  const saveDeck = async () => {
    ...
    setSaving(true);
    try {
      const { data: deck } = await supabase.from('flashcard_decks').insert({ student_id: 'brynne', title: deckName.trim(), card_count: cards.length, folder_id: folderId || null }).select().single();
      if (deck) {
