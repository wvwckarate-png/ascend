  ...
  const generate = async () => {
    ...
    try {
      ...
      const baseInstruction = totalSelected > 1
        ? `You are Ascend analyzing ${allFiles.length} documents for Brynne, an advanced 5th grader who does high school level math and science. Use friendly, encouraging language. Perform CROSS-DOCUMENT ANALYSIS: identify concepts recurring across multiple documents, find overlapping themes. Generate ${count} flashcards focused on these key concepts.${topic.trim() ? ` Additional focus: ${topic.trim()}.` : ''}`
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAFAF8' }}>
        ...
        <main style={{ maxWidth: 600, margin: '0 auto', padding: '28px 20px 80px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
            <div>
              ...
              <div style={{ fontSize: 28, fontWeight: 800, color: '#1D1B26', letterSpacing: '-0.8px' }}>My Decks 🃏</div>
            </div>
            <button onClick={() => setScreen('generate')} style={{ padding: '10px 18px', borderRadius: 999, background: color, border: 'none', color: 'white', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
              + New Deck
            </button>
          </div>

          {decksLoading ? (
            <div style={{ textAlign: 'center', padding: '40px 0', color: '#9E9BB0', fontSize: 13 }}>Loading your decks...</div>
          ) : decks.length === 0 ? (
            <div style={{ background: '#FFFFFF', border: '1.5px dashed #C4C1D4', borderRadius: 18, padding: '48px 20px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 13, color: '#9E9BB0', marginBottom: 24 }}>Make your first flashcard deck and start learning! 🌟</div>
              <button onClick={() => setScreen('generate')} style={{ padding: '12px 24px', borderRadius: 999, background: color, border: 'none', color: 'white', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
                Make First Deck 🌟
              </button>
            </div>
          ) : (
      ...
      {screen === 'deck-detail' && activeDeck && (
        <main style={{ maxWidth: 600, margin: '0 auto', padding: '28px 20px 80px' }}>
          <button onClick={() => setScreen('decks')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: '#6B6880', fontFamily: 'var(--font-jakarta)', marginBottom: 20, padding: 0 }}>← My Decks</button>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 24 }}>
            ...
            <button onClick={() => { setShowAddCard(true); setEditCardId(null); setNewFront(''); setNewBack(''); }} style={{ padding: '13px 18px', borderRadius: 14, border: `1.5px solid ${color}`, background: light, color, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>+ Add Card</button>
          </div>
              ...
              {deckCards.map((card, i) => (
                <div key={card.id || i} style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 14, padding: '14px 16px', boxShadow: '0 1px 4px rgba(29,27,38,0.05)' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: '#C4C1D4', marginBottom: 4 }}>Question ❓</div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: '#1D1B26', marginBottom: 8, lineHeight: 1.4 }}>{card.front}</div>
                      <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color, marginBottom: 4 }}>Answer 🌟</div>
                      <div style={{ fontSize: 13, color: '#1D1B26', lineHeight: 1.4 }}>{card.back}</div>
                    </div>
          ...
          {showAddCard && (
            <div onClick={e => { if (e.target === e.currentTarget) { setShowAddCard(false); setEditCardId(null); setNewFront(''); setNewBack(''); }}} style={{ position: 'fixed', inset: 0, background: 'rgba(29,27,38,0.5)', backdropFilter: 'blur(4px)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
              <div style={{ background: '#FFFFFF', borderRadius: '22px 22px 0 0', padding: '24px 20px 44px', width: '100%', maxWidth: 580, boxShadow: '0 -8px 40px rgba(29,27,38,0.15)' }}>
                ...
                <div style={{ fontSize: 18, fontWeight: 800, color: '#1D1B26', marginBottom: 16 }}>{editCardId ? 'Edit Card ✏️' : 'Add a Card! 📝'}</div>
                <div style={{ marginBottom: 12 }}>
                  <label style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase' as const, color: '#9E9BB0', marginBottom: 6, display: 'block' }}>Question (Front)</label>
                ...
                <div style={{ marginBottom: 20 }}>
                  <label style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase' as const, color: '#9E9BB0', marginBottom: 6, display: 'block' }}>Answer (Back)</label>
                  <textarea value={newBack} onChange={e => setNewBack(e.target.value)} rows={3} placeholder="What's the answer?" style={{ width: '100%', padding: '11px 13px', border: `1.5px solid ${color}40`, borderRadius: 10, fontFamily: 'var(--font-jakarta)', fontSize: 14, color: '#1D1B26', background: '#FAFAF8', outline: 'none', resize: 'vertical', lineHeight: 1.5, boxSizing: 'border-box' as const }} />
                </div>
      ...
      {screen === 'generate' && (
        <main style={{ maxWidth: 600, margin: '0 auto', padding: '28px 20px 80px' }}>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 13, color: '#9E9BB0' }}>Pick your files and Ascend will make awesome flashcards!</div>
          </div>
        ...
        <main style={{ maxWidth: 600, margin: '0 auto', padding: '20px 20px 80px' }}>
          {showSave && !saved && (
            <div style={{ background: light, border: `1.5px solid ${color}40`, borderRadius: 14, padding: '14px 16px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ flex: 1 }}>
                ...
                <input value={deckName} onChange={e => setDeckName(e.target.value)} placeholder="Name your deck..." style={{ width: '100%', padding: '8px 11px', border: `1.5px solid ${color}40`, borderRadius: 8, fontFamily: 'var(--font-jakarta)', fontSize: 13, color: '#1D1B26', background: '#FFFFFF', outline: 'none' }} />
              </div>
          ...
          {saved && (
            <div style={{ background: '#EDF7F2', borderRadius: 12, padding: '10px 14px', marginBottom: 16, fontSize: 12, fontWeight: 700, color: '#5FAD8E' }}>
              ✅ Deck saved — find it in My Decks!
            </div>
          )}
      ...
      {screen === 'done' && (
        <main style={{ maxWidth: 500, margin: '0 auto', padding: '40px 20px 80px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, maxWidth: 340, margin: '0 auto 28px' }}>
            {[{ n: Object.keys(ratings).length, l: 'Reviewed', c: color }, { n: knewWell, l: 'Knew Well 🌟', c: '#5FAD8E' }, { n: needWork, l: 'Keep Going!', c: '#C47878' }, { n: cards.length, l: 'Total Cards', c: '#C8965A' }].map((s, i) => (
          ...
          {!saved && (
            <div style={{ background: light, borderRadius: 14, padding: '16px', marginBottom: 20, maxWidth: 340, margin: '0 auto 20px' }}>
              ...
              <input value={deckName} onChange={e => setDeckName(e.target.value)} placeholder="Name your deck..." style={{ width: '100%', padding: '9px 12px', border: `1.5px solid ${color}40`, borderRadius: 9, fontFamily: 'var(--font-jakarta)', fontSize: 13, color: '#1D1B26', background: '#FFFFFF', outline: 'none', marginBottom: 10 }} />
          ...
          {saved && (
            <div style={{ background: '#EDF7F2', borderRadius: 12, padding: '12px 16px', marginBottom: 20, maxWidth: 340, margin: '0 auto 20px' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#5FAD8E' }}>✅ Deck saved to My Decks!</div>
            </div>
          )}
