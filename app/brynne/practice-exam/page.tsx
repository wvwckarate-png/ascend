...
function BrynnePracticeExamInner() {
  ...
  const buildPrompt = (fileCount: number) => {
    ...
    return `You are Ascend generating a practice exam. ${studentCtx} ${classCtx}\n\n${goalCtx}${topic.trim() ? ` Topic focus: ${topic.trim()}.` : ''} ${crossDoc}Generate ${countStr} questions of these types: ${typeDescriptions}. ${typeList.length > 1 ? 'Distribute evenly across all types.' : ''}${weakSpotsInject}${chemInject}${custom}\n\nReturn ONLY a JSON array, no markdown, no backticks. Use these exact formats:\n${formats}`;
  };
  ...
  const submitExam = async () => {
    ...
    const items = questions.map((q, i) => {
      ...
      return { exam_id: examId, student_id: 'brynne', folder_id: activeExam?.folder_id || null, question_index: i, question_type: q.type, question_text: q.question, is_correct: isCorrect };
    });
