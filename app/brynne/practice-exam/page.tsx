  ...
  const generate = async () => {
    ...
    try {
      ...
      if (allFiles.length > 0) {
        ...
        const res = await fetch('/api/generate-study-guide', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt, student: 'brynne', type: 'exam' }) });
        const d = await res.json(); raw = (d.studyGuide || d.content || '').replace(/```json/g, '').replace(/```/g, '').trim();
      }
      ...
      const { data: examData } = await supabase.from('practice_exams').insert({ student_id: 'brynne', title, questions: parsed, responses: {}, status: 'in_progress', timer_seconds: timerEnabled ? timerMinutes * 60 : null }).select().single();
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                        ...
                        <span style={{ fontSize: 11, fontWeight: 700, color: exam.status === 'completed' ? '#5FAD8E' : color, background: exam.status === 'completed' ? '#EDF7F2' : light, padding: '2px 8px', borderRadius: 999 }}>{exam.status === 'completed' ? 'Done! 🌟' : 'In Progress'}</span>
                    ...
                    <button onClick={() => { if (confirm('Delete this test?')) deleteExam(exam.id); }} style={{ fontSize: 11, fontWeight: 700, color: '#C47878', background: '#FDF2F2', border: 'none', borderRadius: 8, padding: '5px 10px', cursor: 'pointer', fontFamily: 'var(--font-jakarta)', flexShrink: 0, marginLeft: 10 }}>Delete</button>
                  </div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => openExam(exam)} style={{ flex: 1, padding: '9px', borderRadius: 10, border: 'none', background: color, color: 'white', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>{exam.status === 'completed' ? 'View Results' : 'Continue'}</button>
