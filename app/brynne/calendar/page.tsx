...
export default function BrynneCalendar() {
  ...
  const fetchTasks = async () => {
    ...
    const { data } = await supabase.from('tasks').select('*').eq('student_id', 'brynne').gte('due_date', start).lte('due_date', end).order('due_time', { ascending: true });
  ...
  const addTask = async () => {
    ...
    setSaving(true);
    await supabase.from('tasks').insert({ student_id: 'brynne', title: newTaskTitle.trim(), due_date: selectedDate, due_time: newTaskTime || null, class_name: newTaskClass || null, task_type: 'task' });
    ...
    <div style={{ minHeight: '100vh', background: '#FAFAF8' }}>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '20px 20px 80px' }}>
        ...
        <div style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 18, padding: '16px', marginBottom: 16, boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2 }}>
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div key={day} onClick={() => setSelectedDate(dateStr)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '6px 2px', borderRadius: 10, cursor: 'pointer', background: isSelected ? '#E8956D' : isToday ? '#FFF3E8' : 'transparent' }}>
                  <span style={{ fontSize: 13, fontWeight: isToday || isSelected ? 800 : 400, color: isSelected ? 'white' : isToday ? '#E8956D' : '#1D1B26', marginBottom: 3 }}>{day}</span>
                  {hasTasks && (
                    <div style={{ display: 'flex', gap: 2 }}>
                      {hasIncomplete && <div style={{ width: 4, height: 4, borderRadius: '50%', background: isSelected ? 'rgba(255,255,255,0.7)' : '#E8956D' }} />}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <button onClick={() => setShowAddTask(true)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '7px 14px', borderRadius: 999, border: 'none', background: '#E8956D', color: 'white', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>+ Add Task ✨</button>
        </div>
        ...
        {showAddTask && (
          <div style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 16, padding: '16px', marginBottom: 12, boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
            <input value={newTaskTitle} onChange={e => setNewTaskTitle(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') addTask(); }} placeholder="What do you need to do? ✨" autoFocus style={{ width: '100%', padding: '10px 13px', border: '1.5px solid #E8E5F0', borderRadius: 10, fontFamily: 'var(--font-jakarta)', fontSize: 14, color: '#1D1B26', background: '#FAFAF8', outline: 'none', marginBottom: 8 }} />
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={addTask} disabled={!newTaskTitle.trim() || saving} style={{ flex: 1, padding: '10px', borderRadius: 10, border: 'none', background: '#E8956D', color: 'white', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)', opacity: !newTaskTitle.trim() ? 0.4 : 1 }}>{saving ? 'Saving...' : 'Add Task! ✨'}</button>
        ...
        {selectedTasks.length === 0 ? (
          <div style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 16, padding: '24px', textAlign: 'center', boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
            ...
            <div style={{ fontSize: 13, color: '#9E9BB0' }}>Nothing here yet! Add something fun to do! ✨</div>
          </div>
        ) : (
