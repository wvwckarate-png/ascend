...
export default function BrynneCalendar() {
  ...
  const handleAddTask = async () => {
    ...
    const { error } = await supabase.from('tasks').insert({
      student_id: 'brynne', title: newTitle.trim(), due_date: newDate,
      due_time: newTime || null, task_type: newType, class_name: newClass || null, completed: false
    });
    if (!error) {
      const { data: fresh } = await supabase.from('tasks').select('*').eq('student_id', 'brynne').order('due_date', { ascending: true });
