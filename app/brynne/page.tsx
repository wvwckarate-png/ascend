  ...
  useEffect(() => {
    const load = async () => {
      const [{ data: studentData }, { data: classData }, { data: taskData }, { data: folderData }] = await Promise.all([
        supabase.from('students').select('name, grade, focus').eq('id', 'brynne').single(),
        supabase.from('classes').select('id, name, semester, professor').eq('student_id', 'brynne').eq('is_active', true).order('created_at', { ascending: false }),
    ...
    <div style={{ minHeight: '100vh', background: '#FAFAF8' }}>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '24px 20px 80px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 20 }}>
          {[
            { label: 'Study Guide',   emoji: '📖', path: '/brynne/study' },
            { label: 'Flashcards',    emoji: '🗂️', path: '/brynne/flashcards' },
            { label: 'Practice Test', emoji: '📝', path: '/brynne/practice-exam' },
            { label: 'Classes',       emoji: '📚', path: '/brynne/classes' },
          ].map(tool => (
            <div
              key={tool.label}
              onClick={() => router.push(tool.path)}
              style={{ background: '#FFFFFF', border: `1.5px solid ${color}20`, borderRadius: 14, padding: '14px 8px', textAlign: 'center', cursor: 'pointer', boxShadow: '0 1px 4px rgba(29,27,38,0.06)', transition: 'transform 0.15s' }}
              ...
              {classes.slice(0, 4).map((cls, i) => {
                ...
                return (
                  <div
                    ...
                    onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.transform = 'translateY(0)'}
                  >
                    <div style={{ width: 32, height: 32, borderRadius: 9, background: c + '22', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: c, marginBottom: 8 }}>
                      {classLabel(cls.name)}
                    </div>
        ...
        <div style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 18, padding: '18px 20px', marginBottom: 16, boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <Link href="/brynne/calendar" style={{ fontSize: 10, fontWeight: 700, color, textDecoration: 'none' }}>Calendar →</Link>
            </div>
          </div>
            ...
            <>
              ...
              {upcomingTasks.length === 0 && overdueTasks.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '24px 0' }}>
                  ...
                  <div style={{ fontSize: 12, color: '#9E9BB0' }}>Nothing left! You're amazing! Add a task from the calendar.</div>
                </div>
              ) : (
