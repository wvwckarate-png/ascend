  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAFAF8' }}>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '28px 20px 80px' }}>
        <button onClick={() => router.push(`/brynne/classes/${classId}`)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: '#6B6880', fontFamily: 'var(--font-jakarta)', marginBottom: 20, padding: 0 }}>
          ← {cls?.name || 'Back'}
        </button>
          ...
          <div style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 18, overflow: 'hidden', boxShadow: '0 1px 6px rgba(29,27,38,0.06)', marginBottom: 20 }}>
            <div style={{ padding: '20px 20px 0' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
                <div style={{ width: 46, height: 46, borderRadius: 12, background: light, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color, flexShrink: 0 }}>
                  {classLabel(cls.name)}
                </div>
              ...
              <div style={{ display: 'flex', borderBottom: '1.5px solid #E8E5F0' }}>
                {TABS.map(t => (
                  <div key={t.key} onClick={() => setTab(t.key)} style={{ padding: '10px 16px', fontSize: 11, fontWeight: 700, cursor: 'pointer', borderBottom: `2.5px solid ${tab === t.key ? color : 'transparent'}`, marginBottom: -1.5, color: tab === t.key ? color : '#C4C1D4', whiteSpace: 'nowrap' }}>
                    {t.label}
                  </div>
                ))}
              </div>
            ...
            <div style={{ padding: '20px' }}>
              ...
              {tab === 'resources' && (
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                    <div style={{ fontSize: 13, color: '#9E9BB0', lineHeight: 1.5 }}>Notes, worksheets, audio, links — everything for this unit! 📚</div>
                    <button onClick={() => setShowUpload(true)} style={{ padding: '8px 16px', borderRadius: 999, background: color, border: 'none', color: 'white', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)', flexShrink: 0, marginLeft: 12 }}>
                      + Upload
                    </button>
                  </div>
                  {resources.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '32px 0', border: '2px dashed #E8E5F0', borderRadius: 12 }}>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 700, color: '#1D1B26', marginBottom: 6 }}>Nothing here yet!</div>
                      <div style={{ fontSize: 12, color: '#9E9BB0', marginBottom: 18 }}>Upload your notes or worksheets to get started. 🌟</div>
                      <button onClick={() => setShowUpload(true)} style={{ padding: '10px 22px', borderRadius: 999, background: color, border: 'none', color: 'white', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
                        Upload Something
                      </button>
                    </div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {resources.map(r => (
                        <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: '#FAFAF8', borderRadius: 12, border: '1.5px solid #E8E5F0' }}>
                          ...
                          {r.storage_url && <a href={r.storage_url} target="_blank" rel="noreferrer" style={{ fontSize: 11, fontWeight: 700, color, textDecoration: 'none' }}>Open</a>}
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {tab === 'guide' && (
                <div style={{ textAlign: 'center', padding: '32px 0' }}>
                  ...
                  <div style={{ fontSize: 13, color: '#9E9BB0', marginBottom: 20, lineHeight: 1.6 }}>
                    Upload your notes first — Ascend will build you an awesome study guide! 🌟
                  </div>
                  <button onClick={() => router.push('/brynne/study')} style={{ padding: '10px 22px', borderRadius: 999, background: resources.length > 0 ? color : '#F3F1EC', border: 'none', color: resources.length > 0 ? 'white' : '#9E9BB0', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
                    {resources.length > 0 ? 'Generate Study Guide ✨' : 'Upload Notes First'}
                  </button>
                </div>
              )}
              ...
              {tab === 'cards' && (
                <div style={{ textAlign: 'center', padding: '32px 0' }}>
                  ...
                  <div style={{ fontSize: 13, color: '#9E9BB0', marginBottom: 20, lineHeight: 1.6 }}>
                    Ascend can make flashcards from your notes — super helpful for studying! 🎉
                  </div>
                  <button onClick={() => router.push('/brynne/flashcards')} style={{ padding: '10px 22px', borderRadius: 999, background: resources.length > 0 ? color : '#F3F1EC', border: 'none', color: resources.length > 0 ? 'white' : '#9E9BB0', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
                    {resources.length > 0 ? 'Generate Flashcards 🃏' : 'Upload Notes First'}
                  </button>
                </div>
              )}
              ...
              {tab === 'exam' && (
                <div style={{ textAlign: 'center', padding: '32px 0' }}>
                  ...
                  <div style={{ fontSize: 13, color: '#9E9BB0', marginBottom: 20, lineHeight: 1.6 }}>
                    Practice tests help you get ready — let's make one from your notes! ✨
                  </div>
                  <button onClick={() => router.push('/brynne/practice-exam')} style={{ padding: '10px 22px', borderRadius: 999, background: resources.length > 0 ? color : '#F3F1EC', border: 'none', color: resources.length > 0 ? 'white' : '#9E9BB0', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
                    {resources.length > 0 ? 'Generate Practice Test 📝' : 'Upload Notes First'}
                  </button>
                </div>
              )}
        ...
        <div onClick={e => { if (e.target === e.currentTarget) { setShowUpload(false); setUpType(''); setUpName(''); setUpLink(''); }}} style={{ position: 'fixed', inset: 0, background: 'rgba(29,27,38,0.45)', backdropFilter: 'blur(4px)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div style={{ background: '#FFFFFF', borderRadius: '22px 22px 0 0', padding: '24px 20px 36px', width: '100%', maxWidth: 580, boxShadow: '0 -8px 40px rgba(29,27,38,0.12)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8, marginBottom: 20 }}>
              {FILE_TYPES.map(ft => (
                <div key={ft.key} onClick={() => setUpType(ft.key)} style={{ padding: '10px 6px', borderRadius: 12, border: `1.5px solid ${upType === ft.key ? color : '#E8E5F0'}`, background: upType === ft.key ? light : '#FAFAF8', cursor: 'pointer', textAlign: 'center' }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input
                ...
                placeholder='e.g. "Chapter 7 Notes" or "Old Quiz"'
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
              ...
              <button onClick={handleAddResource} disabled={!upName.trim() || !upType || saving} style={{ flex: 2, padding: '12px', borderRadius: 12, border: 'none', background: color, color: 'white', fontSize: 13, fontWeight: 800, cursor: 'pointer', fontFamily: 'var(--font-jakarta)', opacity: !upName.trim() || !upType || saving ? 0.4 : 1 }}>
