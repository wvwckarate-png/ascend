...
function MichaelPracticeExamInner() {
  ...
  const buildPrompt = (fileCount: number) => {
    ...
    return `You are Ascend generating a practice exam for Michael, a pre-med 9th grader building MCAT foundations.${topic.trim() ? ` Topic: ${topic.trim()}.` : ''} ${crossDoc}Generate ${countStr} questions of these types: ${typeDescriptions}. ${typeList.length > 1 ? 'Distribute evenly across all types.' : ''} Make questions challenging and exam-realistic.${custom}\n\nReturn ONLY a JSON array, no markdown, no backticks. Use these exact formats:\n${formats}`;
  };
              ...
              {pastExams.map(exam => (
                <div key={exam.id} style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 16, padding: '18px 20px', boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                        ...
                        <span style={{ fontSize: 11, fontWeight: 700, color: exam.status === 'completed' ? '#5FAD8E' : color, background: exam.status === 'completed' ? '#EDF7F2' : light, padding: '2px 8px', borderRadius: 999 }}>{exam.status === 'completed' ? 'Completed' : 'In Progress'}</span>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => openExam(exam)} style={{ flex: 1, padding: '9px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #7B6FA0, #5A5078)', color: 'white', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>{exam.status === 'completed' ? 'View Results' : 'Continue'}</button>
            ...
            <div style={{ background: light, borderRadius: 14, padding: '16px', marginTop: 24 }}>
              ...
              {scheduleReview && (
                <button onClick={async () => {
                  ...
                  const tasks = [1, 3, 7].map(d => { const due = new Date(today); due.setDate(today.getDate() + d); return { student_id: 'michael', title: `Review: ${examTitle}`, due_date: due.toISOString().split('T')[0], task_type: 'review', completed: false }; });
