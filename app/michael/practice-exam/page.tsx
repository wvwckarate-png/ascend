...
function MichaelPracticeExamInner() {
  ...
  const buildPrompt = (fileCount: number) => {
    ...
    const weakAreasInject = selectedWeakList.length > 0
      ? ` PRIORITY FOCUS — Michael has previously missed questions on these specific concepts (miss count in parentheses): ${selectedWeakList.map((w, i) => `${i + 1}. ${w.question.slice(0, 120)}${w.missCount > 1 ? ` (missed ${w.missCount}x)` : ''}`).join('; ')}. Weight at least half of the questions toward these areas, approaching them from fresh angles to test genuine understanding.`
      : '';
    const weakSpotsInject = weakSpotsList.length > 0
      ? ` PRIORITY FOCUS — These are Michael's confirmed weak spots from prior study sessions: ${weakSpotsList.map((w, i) => `${i + 1}. ${w}`).join('; ')}. Weight at least half of the questions toward these specific concepts, approaching them from fresh angles to test genuine understanding.`
      : '';
      ...
      {screen === 'setup' && (
        <main style={{ maxWidth: 600, margin: '0 auto', padding: '28px 20px 80px' }}>
          ...
          {folderId && (weakAreasLoading || weakAreas.length > 0) && (
            <div style={{ background: '#FDF2F2', border: '1.5px solid #C4787840', borderRadius: 18, padding: '16px 20px', marginBottom: 12, boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: weakAreasExpanded ? 12 : 0 }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 800, color: '#C47878' }}>Your Weak Areas {weakAreas.length > 0 && `— ${folderName || 'This Folder'}`}</div>
                  {weakAreas.length > 0 && <div style={{ fontSize: 11, color: '#9E9BB0', marginTop: 2 }}>{selectedWeakAreas.size} of {weakAreas.length} selected · AI will target these in your exam</div>}
                </div>
                      ...
                      return (
                        <div key={i} onClick={() => setSelectedWeakAreas(prev => { const n = new Set(prev); n.has(i) ? n.delete(i) : n.add(i); return n; })} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', borderRadius: 10, border: `1.5px solid ${selected ? '#C47878' : '#E8E5F0'}`, background: selected ? '#FDF2F2' : '#FAFAF8', cursor: 'pointer' }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            {w.missCount > 1 && <div style={{ fontSize: 10, fontWeight: 700, color: '#C47878', marginTop: 3 }}>Missed {w.missCount}x</div>}
                          </div>
                        </div>
                      );
                    })}
          ...
          <div style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 18, padding: '20px', marginBottom: 12, boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
            ...
            <input value={examNameInput} onChange={e => setExamNameInput(e.target.value)} placeholder='e.g. "Bio 101 Midterm Practice"' style={{ ...inputStyle }} />
          </div>
          ...
          <div style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 18, padding: '20px', marginBottom: 12, boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
            ...
            <input value={topic} onChange={e => setTopic(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && canGenerate && !generating) generate(); }} placeholder={totalSelected > 0 ? 'e.g. "Focus on enzyme kinetics"' : 'e.g. Biology - Cell Division'} style={{ ...inputStyle, marginBottom: 14 }} />
            ...
            <textarea value={customInstructions} onChange={e => setCustomInstructions(e.target.value)} placeholder='e.g. "Weight toward mechanisms" or "Include clinical applications"' rows={2} style={{ ...inputStyle, resize: 'vertical', lineHeight: 1.5, marginBottom: 14 }} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {questions.map((q, i) => (
              <div key={i} style={{ background: '#FFFFFF', border: '1.5px solid #E8E5F0', borderRadius: 18, padding: '22px 24px', boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
                ...
                {q.type === 'sa' && <textarea value={responses[i] || ''} onChange={e => setResponses(r => ({ ...r, [i]: e.target.value }))} placeholder="Write your answer here..." rows={4} style={{ width: '100%', padding: '12px 14px', border: '1.5px solid #E8E5F0', borderRadius: 12, fontFamily: 'var(--font-jakarta)', fontSize: 14, color: '#1D1B26', background: '#FAFAF8', outline: 'none', resize: 'vertical', lineHeight: 1.6, boxSizing: 'border-box' }} />}
                {q.type === 'essay' && <textarea value={responses[i] || ''} onChange={e => setResponses(r => ({ ...r, [i]: e.target.value }))} placeholder="Write your essay response here..." rows={8} style={{ width: '100%', padding: '12px 14px', border: '1.5px solid #E8E5F0', borderRadius: 12, fontFamily: 'var(--font-jakarta)', fontSize: 14, color: '#1D1B26', background: '#FAFAF8', outline: 'none', resize: 'vertical', lineHeight: 1.6, boxSizing: 'border-box' }} />}
              </div>
            ))}
          </div>
          ...
          <div style={{ background: 'linear-gradient(135deg, #7B6FA0, #5A5078)', borderRadius: 20, padding: '28px 24px', marginBottom: 24, color: 'white', textAlign: 'center' }}>
            ...
            {displayScore !== null ? (
              <>
                ...
                {questions.filter(q => q.type === 'sa' || q.type === 'essay').length > 0 && <div style={{ fontSize: 12, opacity: 0.7, marginTop: 6 }}>{questions.filter(q => q.type === 'sa' || q.type === 'essay').length} open-ended questions — review below</div>}
              </>
            ) : (
              <>
                ...
                <div key={i} style={{ background: '#FFFFFF', border: `1.5px solid ${isCorrect === true ? '#5FAD8E40' : isCorrect === false ? '#C4787840' : '#E8E5F0'}`, borderRadius: 18, padding: '20px 22px', boxShadow: '0 1px 6px rgba(29,27,38,0.06)' }}>
                  ...
                  {(q.type === 'mc' || q.type === 'tf') && (
                    <div>
                      ...
                      {showExp && <div style={{ background: light, border: `1.5px solid ${color}30`, borderRadius: 10, padding: '12px 14px' }}><div style={{ fontSize: 13, color: '#5A5078', lineHeight: 1.6 }}>{(q as MCQuestion | TFQuestion).explanation}</div></div>}
                    </div>
                  )}
                  {(q.type === 'sa' || q.type === 'essay') && (
                    <div style={{ background: light, border: `1.5px solid ${color}30`, borderRadius: 10, padding: '12px 14px' }}>
                      ...
                      <div style={{ fontSize: 13, color: '#5A5078', lineHeight: 1.6 }}>{q.type === 'sa' ? (q as SAQuestion).model_answer : (q as EssayQuestion).key_points}</div>
                    </div>
                  )}
                </div>
              );
