...
function MichaelFlashcardsInner() {
  ...
  const fetchClassMeta = async (fId: string) => {
    const [{ data: folder }, { data: student }] = await Promise.all([
      ...
      supabase.from('students').select('name, grade, track, target_school, target_program, grad_year, generation_profile').eq('id', 'michael').single(),
    ]);
    if (!folder) return;
    ...
    if (cls) setClassMeta({
      ...
      studentName: student?.name || 'Michael',
      studentGrade: student?.grade || '9th grade',
      ...
      studentGradYear: student?.grad_year ? String(student.grad_year) : '2028',
  ...
  const openDeck = async (deck: Deck) => {
    ...
    if (data) {
      ...
      if (cardIds.length > 0) {
        const { data: items } = await supabase.from('flashcard_items').select('*').in('card_id', cardIds).eq('student_id', 'michael');
  ...
  const recordRating = async (cardId: string, deckId: string, correct: boolean) => {
    ...
    if (existing) {
      ...
      const nextReview = new Date(); nextReview.setDate(nextReview.getDate() + newInterval);
      const { data } = await supabase.from('flashcard_items').insert({ card_id: cardId, deck_id: deckId, student_id: 'michael', times_seen: 1, times_correct: correct ? 1 : 0, times_missed: correct ? 0 : 1, last_seen: now, next_review: nextReview.toISOString(), interval_days: newInterval, is_retired: false }).select().single();
  ...
  const markAsMastered = async (deckId: string, deckCards: Card[]) => {
    ...
    await supabase.from('flashcard_items').update({ is_retired: true }).in('card_id', cardIds).eq('student_id', 'michael');
  ...
  const uploadCardImage = async (file: File, cardId: string, side: 'front' | 'back'): Promise<string | null> => {
    try {
      ...
      const path = `michael/${cardId}_${side}.jpg`;
  ...
  const saveDeck = async () => {
    ...
    setSaving(true);
    try {
      const { data: deck } = await supabase.from('flashcard_decks').insert({ student_id: 'michael', title: deckName.trim(), card_count: cards.length, folder_id: selectedFolderId || folderId || null }).select().single();
      if (deck) {
        ...
        if (scheduleReview) {
          ...
          const reviewTasks = [1, 3, 7].map(d => {
            ...
            return { student_id: 'michael', title: `Review: ${deckName.trim()} flashcards`, due_date: due.toISOString().split('T')[0], task_type: 'review', completed: false, resource_id: deck.id, resource_type: 'flashcard_deck' };
          });
  ...
  const saveBuildDeck = async () => {
    ...
    try {
      ...
      const { data: deck } = await supabase.from('flashcard_decks').insert({ student_id: 'michael', title: buildDeckName.trim(), card_count: validCards.length, folder_id: buildFolderId || null }).select().single();
      if (deck) {
  ...
  <button onClick={() => setScreen('generate')} style={{ padding: '10px 18px', borderRadius: 999, background: 'linear-gradient(135deg, #7B6FA0, #5A5078)', border: 'none', color: 'white', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>+ Generate</button>
      ...
      {screen === 'deck-detail' && activeDeck && (
        <main style={{ maxWidth: 600, margin: '0 auto', padding: '28px 20px 80px' }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
            <button onClick={() => { if (deckCards.length > 0) studyDeck(activeDeck, deckCards, false); }} disabled={deckCards.length === 0} style={{ flex: 1, padding: '13px', borderRadius: 14, border: 'none', background: 'linear-gradient(135deg, #7B6FA0, #5A5078)', color: 'white', fontSize: 13, fontWeight: 800, cursor: 'pointer', fontFamily: 'var(--font-jakarta)', opacity: deckCards.length === 0 ? 0.4 : 1 }}>Study Full Deck</button>
                ...
                {lightboxSide === 'front' && lightboxCard.front_image_url ? (
                  <div style={{ position: 'relative' }}>
                    ...
                    {(lightboxCard.front_blocks || []).length > 0 && (
                      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none', borderRadius: 12 }}>
                        {(lightboxCard.front_blocks || []).map((b, i) => <rect key={i} x={`${b.x}%`} y={`${b.y}%`} width={`${b.w}%`} height={`${b.h}%`} fill="#7B6FA0" />)}
                      </svg>
                    )}
                  </div>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    {(lightboxCard.back_blocks || []).length > 0 && (
                      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none', borderRadius: 12 }}>
                        {(lightboxCard.back_blocks || []).map((b, i) => <rect key={i} x={`${b.x}%`} y={`${b.y}%`} width={`${b.w}%`} height={`${b.h}%`} fill="#7B6FA0" />)}
                      </svg>
                    )}
                  </div>
                ) : null}
