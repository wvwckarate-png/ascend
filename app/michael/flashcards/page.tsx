  ...
  const generate = async () => {
    ...
    try {
      ...
      if (folderFiles.length > 0) {
        const prompt = `Generate ${count} flashcards from the uploaded study materials${topic.trim() ? ` about: ${topic}` : ''}. Return ONLY a JSON array with no markdown, no backticks, no explanation. Format: [{"front":"question","back":"answer"}]. Pre-med college level.`;
