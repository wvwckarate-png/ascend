  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAFAF8' }}>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '28px 20px 80px' }}>
        <button onClick={() => router.push('/michael/classes')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: '#6B6880', fontFamily: 'var(--font-jakarta)', marginBottom: 20, padding: 0 }}>
          ← Classes
        </button>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          ...
          <button onClick={() => setShowAdd(true)} style={{ padding: '6px 14px', borderRadius: 999, background: '#EDE9F7', border: 'none', color: '#7B6FA0', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
            + Add Folder
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ background: '#FFFFFF', border: '1.5px dashed #C4C1D4', borderRadius: 18, padding: '40px 20px', textAlign: 'center' }}>
            ...
            <button onClick={() => setShowAdd(true)} style={{ padding: '10px 22px', borderRadius: 999, background: '#7B6FA0', border: 'none', color: 'white', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
              Add First Folder
            </button>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {folders.map((folder) => {
              ...
              return (
                <div
                  key={folder.id}
                  onClick={() => router.push(`/michael/classes/${classId}/${folder.id}`)}
