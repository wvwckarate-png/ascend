...
export default function MichaelStudy() {
  ...
  return (
    <main className="min-h-screen px-6 py-12 max-w-2xl mx-auto">
      ...
      <h1 className="text-4xl mb-1" style={{ color: 'var(--purple-dark)' }}>Generate Study Guide</h1>
      <p className="mb-10" style={{ color: 'var(--text-secondary)' }}>Upload a PDF and Ascend will build your study guide.</p>
      ...
      {!studyGuide ? (
        <>
          <div className="rounded-2xl p-6 mb-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
            ...
            >
              ...
              <input id="pdf-upload-michael" type="file" accept=".pdf" className="hidden" onChange={(e) => setFile(e.target.files?.[0] || null)} />
            </div>
          </div>
          {error && <p className="text-sm mb-4 text-red-500">{error}</p>}
          <button onClick={handleGenerate} disabled={!file || loading} className="w-full py-3 rounded-xl font-medium text-sm transition-all duration-200 hover:opacity-90 disabled:opacity-40" style={{ background: 'var(--purple)', color: 'white' }}>
        ...
        <div className="rounded-2xl p-6" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
          {showNamePrompt && (
            <div className="rounded-xl p-4 mb-6" style={{ background: 'var(--purple-light)', border: '1px solid var(--border)' }}>
              <p className="text-sm font-semibold mb-3" style={{ color: 'var(--purple-dark)' }}>What should we call this study guide?</p>
              <input type="text" value={guideName} onChange={(e) => setGuideName(e.target.value)} className="w-full px-4 py-2 rounded-xl text-sm outline-none mb-3" style={{ background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text-primary)' }} placeholder="e.g. Biology Chapter 5 - Ecosystems" />
              <button onClick={handleSave} disabled={!guideName.trim() || saving} className="w-full py-2 rounded-xl font-medium text-sm transition-all hover:opacity-90 disabled:opacity-40" style={{ background: 'var(--purple)', color: 'white' }}>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => { setStudyGuide(''); setFile(null); setSaved(false); setShowNamePrompt(false); }} className="text-sm" style={{ color: 'var(--purple)' }}>Generate Another</button>
          </div>
          <div className="flex gap-2 mb-6">
            <button onClick={handlePrint} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all hover:opacity-90" style={{ background: 'var(--purple-light)', color: 'var(--purple-dark)' }}>🖨️ Print</button>
            <button onClick={handlePrint} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all hover:opacity-90" style={{ background: 'var(--purple-light)', color: 'var(--purple-dark)' }}>💾 Save as PDF</button>
          </div>
