...
function MichaelStudyInner() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await supabase.from('tasks').insert([1, 3, 7].map(days => ({ student_id: 'michael', title: `Review: ${guideName.trim()}`, due_date: addDays(today, days), task_type: 'review', completed: false })));
