...
function MichaelStudyInner() {
  ...
  const buildPrompt = (fileCount: number) => {
    ...
    const base = fileCount > 1
      ? `You are Ascend analyzing ${fileCount} study documents for Michael, a pre-med freshman in high school beginning to build strong study habits. Perform CROSS-DOCUMENT ANALYSIS: identify concepts recurring across multiple documents, find overlapping themes, and focus your output on these high-frequency areas. ${level ? levelPrompts[level] : ''}`
      : `You are Ascend, an AI study assistant for Michael, a pre-med freshman in high school. Be clear and thorough. ${level ? levelPrompts[level] : 'Generate a study guide based on the instructions below.'}`;
